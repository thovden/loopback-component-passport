...
PassportConfigurator.prototype.configureProvider = function(name, options) {
  ...
  var successRedirect = function(req, accessToken, user) {
    ...
    const redirect = options.successRedirect || (link ? '/link/account' : '/auth/account');

    console.log("redirect", redirect, accessToken, user)
    redirect.replace(/\$\{accessToken\}/, accessToken && accessToken.id)
                   .replace(/\$\{userId\}/, user && user.id);
    return redirect;
  };
      ...
      options.authOptions), function(err, user, info) {
        ...
        if (session) {
          req.logIn(user, function(err) {
            ...
            return res.redirect(successRedirect(req, info && info.accessToken, user));
          });
        } else {
          if (info && info.accessToken) {
            if (!!options.json) {
